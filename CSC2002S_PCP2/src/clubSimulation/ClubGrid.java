	...
	public GridBlock enterClub(PeopleLocation myLocation) throws InterruptedException  {
			...
			synchronized (entrance) {
				while (entrance.occupied()) {
					// Wait if the entrance is occupied
					entrance.wait();
				}
				while (counter.overCapacity()) {
					//wait if the club limit is reached 
					entrance.wait();
				}
				entrance.get(myLocation.getID());
				Thread.sleep(2000);  // Simulate a delay before entering the club (2 seconds)
				counter.personEntered(); //add to counter
				myLocation.setLocation(entrance);
				myLocation.setInRoom(true);
				entrance.notify(); // Notify other waiting threads that the entrance is now not occupied
			}
