...
public class Clubgoer extends Thread {
	...
	Clubgoer( int ID,  PeopleLocation loc,  int speed, AtomicBoolean isPaused) {
		this.ID=ID;
		...
		rand=new Random();
		this.isPaused = isPaused; // Shared flag for pausing and resuming
		}
	...
	public void checkPause() {
		...
		while (isPaused.get()) {
			try {
			isPaused.wait(); // wait while paused
	...
	public void startSim() {
		try { latch.await(); //wait for latch to open
		} catch (InterruptedException e) {} 	
