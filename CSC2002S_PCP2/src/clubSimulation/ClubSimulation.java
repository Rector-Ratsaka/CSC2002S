...
public class  ClubSimulation {
	...
	private static int maxWait=2000; //for the slowest customer
	private static int minWait=1000; //for the fastest customer

	private static CountDownLatch latch; // latch to make threads wait for start/pause button to be clicked
    static AtomicBoolean isPaused = new AtomicBoolean(false); //boolean variable to keep track of the pause state
	...
	public static void setupGUI(int frameX,int frameY,int [] exits) {
	    ...
	    txt.add(scr);
	    g.add(txt);
	    counterDisplay = new CounterDisplay(missed, caught,scr,tallys);      //thread to update score
			...
			pauseB.addActionListener(new ActionListener() {
		      public void actionPerformed(ActionEvent e) {
				if (isPaused.get()) {
					...
					// Notify Clubgoer threads to wake up
	...
	public static void main(String[] args) throws InterruptedException {
        ...
        for (int i=0;i<noClubgoers;i++) {
    			...
    			patrons[i] = new Clubgoer(i,peopleLocations[i],movingSpeed,isPaused);
    		}
		...
		latch = new CountDownLatch(1); // Create the latch
      	for (int i=0;i<noClubgoers;i++) {
			patrons[i].setLatch(latch); // Set the latch for each Clubgoer
			patrons[i].start();
		}
 	}
